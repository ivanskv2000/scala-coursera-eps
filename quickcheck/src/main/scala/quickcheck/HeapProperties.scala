...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val minInt: Int = if (x < y) x else y
      val maxInt: Int = if (x > y) x else y
      val meldedHeap: List[Node] = 
        meld(insert(maxInt, insert(maxInt, empty)), insert(minInt, insert(minInt, empty)))
      ...
      val deleteTwoMinAndFindMin: Boolean =
        findMin(deleteMin(deleteMin(meldedHeap))) == maxInt
      ...
      val insertMinAndFindMin: Boolean =
        findMin(insert(minInt, meldedHeap)) == minInt
  ...
  val meldingHeaps: (String, Prop) =
    def check(heap1: List[Node], heap2: List[Node], meldedHeap: List[Node]): Boolean =
      ...
      if isEmpty(meldedHeap) then 
        ...
        (findMin(heap2) == findMin(meldedHeap)) && check(deleteMin(heap2), heap1, deleteMin(meldedHeap))
      else if isEmpty(heap2) then
        (findMin(heap1) == findMin(meldedHeap)) && check(deleteMin(heap1), heap2, deleteMin(meldedHeap))
      else
        val meldedMin: Int = findMin(meldedHeap)
        if findMin(heap1) == meldedMin then
          val checked: Boolean = check(deleteMin(heap1), heap2, deleteMin(meldedHeap))
          checked
        else if findMin(heap2) == meldedMin then
          val checked: Boolean = check(heap1, deleteMin(heap2), deleteMin(meldedHeap))
          checked
        else
          false
